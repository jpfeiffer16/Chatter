...
var taskbar = {
	...
	loadDefaultPrograms : function () {
		var defaulPrograms = ['Code Editor', 'Desktop Settings', 'Commander', 'Data Transfer', 'Chatter']; 
	...
	getText : function(keyString) {
        ...
        switch(keyString) {
        	...
        	case 'pgrm-Commander':
        		...
        		break;
			case 'pgrm-Data_Transfer':
				return 'dataTransfer(thisWindow, contentArea)';
				break;
			case 'pgrm-Chatter':
				return 'chatter(thisWindow, contentArea);';
				break;
        	default: 
...
function dataTransfer(thisWindow, contentArea) {
	var html = '<h5>Data To Send:</h5>' +
		'<input id="bttxt-data"type="text">' +
		'<button id="dtbtn-send-data">Send Data</button>';
	contentArea.append(html);
	$('#dtbtn-send-data').on('click', function() {
		var dataToSend = $('#bttxt-data').val();
		$.post('http://localhost:1337',{type: 'write', instanceName: storageManager.getText('stng-instance-name'), message: dataToSend})
		.done(function(data) {
			alert('data loaded: ' + data);
		});
	});
	setInterval(function(){
		$.post('http://localhost:1337',{type: 'read', instanceName: storageManager.getText('stng-instance-name')})
		.done(function (data) {
			alert(data);
		});
	}, 3000);
...
function chatter(thisWindow, contentArea) {
	var serverEvents = new EventSource('/sse');
	var windowWidth = 400,
		windowHeight = 600;
	thisWindow.width(windowWidth);
	thisWindow.height(windowHeight);
	windowManager.initWindow('Chatter', 'pgrm', true);
	
	var html = '<ul id="chatter-message-list"></ul>' +
		'<input type="text" id="chatter-message-input" style="position:absolute;bottom:4px;">';
	contentArea.append(html);
	var messageArea = $('#chatter-message-list');
	
	serverEvents.onmessage = function(event) {
		messageArea.append('<li>' + event.data + '</li>');
	};
}